...
/**
 * Pass/fail check implementation that works on asynchronous responses. 
 * Checks the request tracking id matches the response tracking id ref
 *
 * @author Simon Farrow simon.farrow1@hscic.gov.uk
 */
public class AsynchronousTrackingIDTrackingIDRefPassFailCheck
        extends AbstractAsynchronousRequestResponseComparatorPassFailCheck {
    ...
    public void init(PassFailCheckContext passfailCheckCtx)
        ...
        try {
            ...
            trackingIDRefXpath = "//itk:DistributionEnvelope/itk:payloads/itk:payload[1]/itk:InfrastructureResponse/@trackingIdRef";
            trackingIDRefExpression = getXpathExtractor(trackingIDRefXpath);
    ...
    protected boolean doChecks(Script s, InputSource request, InputSource response)
        ...
        String responseTrackingIdRef = trackingIDRefExpression.evaluate(response);
        StringBuilder sb = new StringBuilder();
        if (!Utils.isNullOrEmpty(trackingID) && !Utils.isNullOrEmpty(responseTrackingIdRef)) {

            if (trackingID.compareToIgnoreCase(responseTrackingIdRef)==0) {
                result = true;
                sb.append(colourString("Result: ", BLACK));
                sb.append(colourString("Tracking ID and Response Tracking ID Ref match as expected.", GREEN));
                sb.append(colourString("<BR/>Request: ", BLACK));
                sb.append(colourString(trackingID, GREEN));
                sb.append(colourString("<BR/>Response: ", BLACK));
                sb.append(colourString(responseTrackingIdRef, GREEN));
                sb.append(colourString("<BR/>XPATH: ", BLACK));
                sb.append(colourString(trackingIDRefXpath, GREEN));
            } else {
                result = false;
                sb.append(colourString("Result: ", BLACK));
                sb.append(colourString("Tracking ID and Response Tracking ID Ref unexpectedly do not match.", RED));
                sb.append(colourString("<BR/>Request: ", BLACK));
                sb.append(colourString(trackingID, RED));
                sb.append(colourString("<BR/>Response: ", BLACK));
                sb.append(colourString(responseTrackingIdRef, RED));
                sb.append(colourString("<BR/>XPATH: ", BLACK));
                sb.append(colourString(trackingIDRefXpath, RED));
            }
        } else {
            result = false;
            ...
            if (Utils.isNullOrEmpty(responseTrackingIdRef)) {
                sb.append(colourString("Result: ", BLACK));
                sb.append(colourString("Response Tracking ID Ref is malformed or not present.", RED));
                sb.append(colourString("<BR/>Response: ", BLACK));
                sb.append(colourString(responseTrackingIdRef == null ? "Null" : responseTrackingIdRef, RED));
                sb.append(colourString("<BR/>XPATH: ", BLACK));
                sb.append(colourString(trackingIDRefXpath, RED));
            }
        }
