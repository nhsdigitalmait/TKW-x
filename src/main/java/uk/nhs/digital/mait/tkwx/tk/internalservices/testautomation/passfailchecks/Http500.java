...
/**
 * Synchronous pass/fail check that considers finding an HTTP 500 response, to
...
public class Http500
    ...
    public TestResult passed(Script s, InputStream inResponse, InputStream inRequest)
        ...
        if (httpResponse == null) {
            appendDescription(colourString("HTTP 500.", RED));
        ...
        if (httpResponse.contains("500")) {
            appendDescription(colourString("HTTP 500.", GREEN));
            appendDescription(colourString(" Actual: ", BLACK));
            appendDescription(colourString("HTTP 500.", GREEN));
