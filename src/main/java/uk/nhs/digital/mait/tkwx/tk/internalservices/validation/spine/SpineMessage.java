...
public class SpineMessage {
    ...
    private Boolean SOAPIncludedInHL7Part = false;
     ...
     * @param f filename containing spine message
     * @throws Exception
     */
    public SpineMessage(String sourceDirectory, String f)
            throws Exception {
        filename = f;
     ...
     * @throws Exception
     */
    public SpineMessage(String request)
            throws Exception {
        breakDownMessages(request);
    }

    /**
     * public constructor
     *
     * @param req HttpRequest message
     * @throws Exception
     */
    public SpineMessage(HttpRequest req)
            throws Exception {
        String request = new String(req.getBody());
        breakDownMessages(request);
    }

    private void breakDownMessages(String request) throws Exception {
        // Having read the input, we need to determine what we have in terms of
        // structure, and to split it up into the various parts.

        presentedFile = request.trim();
        // For consistency convert to \n file terminators
        presentedFile = presentedFile.replaceAll("\\r\\n", "\n");
        setPresentationType();
        parse();
        if (service == null) {
            setService();
    ...
    /**
     *
     * @return xml Document object containing ebxml element
     */
    public Document getEbXmlDoc() {
        if (ebXMLpart == null) {
            return null;
        }
        return ebXMLpart.getDocument();
    }

    /**
     * 
     * @return xml Document object containing hl7 
     */
    public Document getHL7Doc() {
        return hl7part.getDocument();
    }

    /**
     * 
     * @return xml Document object  containing soap element
     */
     ...
     * 
     * @param i
     * @return xml Document object containing attachment at given index 
     */
    public Document getATTACHMENTDoc(int i) {
        return attachments.get(i).getDocument();
    }
     ...
     * @param s boolean 
     */
    public void setSOAPIncludedInHL7Part(Boolean s) {
        SOAPIncludedInHL7Part = s;
    }
    ...
    /**
     *
     * @return string containing the HL7 part of the spine message
     */
    ...
    /**
     *
     * @return string containing the SOAP part of the message
     */
    ...
    /**
     *
     * @param i index number into attachment list
     * @return string containing attachment
     */
    public String getATTACHMENTPart(int i) {
        if (attachments.get(i) == null) {
            return null;
        }
        return stripBOM(attachments.get(i).getBody());
    }
    ...
    private String parseHttpHeaders()
        ...
        String ctype = httpHeaders.get(CONTENT_TYPE_HEADER.toUpperCase());
        if (ctype == null) {
    ...
    private void setPresentationType()
        ...
        if ((presentedFile.contains("PartyId")) && (presentedFile.contains("CPAId"))) {
            presentationType = EBXML_MIME;
            return;
        }

        // 3. If "Header" and "ReplyTo" are both present, assume SPINE_SOAP
        if ((presentedFile.contains("Header")) && (presentedFile.contains("ReplyTo"))) {
            presentationType = SPINE_SOAP;
            return;
        }

        // 4. If "versionCode" and "interactionId" are both present, assume BARE_HL7
        if ((presentedFile.contains("versionCode")) && (presentedFile.contains("interactionId"))) {
